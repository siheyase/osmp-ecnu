  ...
  <a-row gutter="16">
    ...
    <a-col :span="8">
      ...
      <a-card title="存证交易" style="height: 560px;">
        ...
        <a-table :columns="taskProcess.tableColumn" :data-source="evidenceTable" bordered :pagination="false" :showHeader="false" />
        <!-- <a-divider /> -->
          ...
          <a-tab-pane key="1">
            ...
            <a-collapse v-model:activeKey="scheduleActiveKey" accordion style="height: 850px; overflow: scroll;">
              ...
              >
                ...
                <a-table :columns="SchduleSlotTableColumn" :data-source="testScheduleSlotTableDatas" style="margin-top: 1%;">
      ...
      <a-card title="可视化结果" style="height: 1000px;">
        ...
        <a-tabs activeKey="1" type="card">
          <a-tab-pane key="1">
            ...
            <BarChart
                  :chartData="taskBarData"
                  :optionConfig="barChartConfig.chartConfig"
                  height="30vh"
                />
          </a-tab-pane>
        </a-tabs>
...
// 创建 Schedule 数据
const testScheduleSlotTableDatas = Array.from({ length: 10 }, testScheduleSlotTableData);

// 生成 10 个 Schedule 对象
const schedules = Array.from({ length: 10 }, (_, index): Schedule => {
  return {
    id: `schedule-${index + 1}`,  // 每个 Schedule 的 id 按序生成
    total: `${(Math.random() * 100).toFixed(2)}`,  // 随机生成总量
    process: `${(Math.random() * 50).toFixed(2)}`,  // 随机生成进度
    nbCommit: `${Math.floor(Math.random() * 100)}`,  // 随机生成提交数量
    nbInvalid: `${Math.floor(Math.random() * 10)}`,  // 随机生成无效数量
    nbNodes: `${Math.floor(Math.random() * 50)}`,  // 随机生成节点数
    tableDatas: testScheduleSlotTableDatas,  // 每个 Schedule 的 tableDatas
  };
});
...
let TaskItem = reactive({
  sign: `0x${Math.floor(Math.random() * 9999999999999)}`, // 任务标识
  total: Math.floor(Math.random() * 100), // 总量
  dataset: 'dataset1', // 数据集
  nbSchedule: Math.floor(Math.random() * 20), // 调度数量，对应task slot
  nbFinalized: Math.floor(Math.random() * 50), // finalized的slot数量
  process: Math.floor(Math.random() * 2), // 已经完成的合成数
})
// 左下角显示的epoch
